...
/*per ricordare se l'ultima operazione effettuata è stata la open con O_CREATE su pathname*/
void resetPath(){
...
int openConnection( const char* sockname, int msec, const struct timespec abstime){
    ...
    while( connect(fd_skt, (struct sockaddr*)&sa, sizeof(sa) ) == -1 ) {
        if ( (difftime(time(NULL), seconds_start)) >= abstime.tv_sec){ //ho superato il tempo massimo
            perror("client : tempo per connessione superato");
            errno = EAGAIN;
            return -1;
        }
...
int readNFiles(int N, const char* dirname) {
    ...
    while (cont < numero_letti) {
        ...
        char * nomecompleto = (char*)malloc((strlen(dirname)+ strlen(nomefile)+ 3)*sizeof(char));
...
int writeFile(const char* pathname, const char* dirname){
    ...
    if (strcmp(path,pathname) != 0 ) {
        perror("L'ultima operazione effettuata con successo non è stata la open con O_CREATE su questo file");
        return -1;
    }
...
int appendToFile(const char* pathname, void* buf, size_t size, const char* dirname){ 
    ...
    if (pathname == NULL || buf == NULL || size < 0) {
        perror("appendToFile: argomento illegale");
        errno = EINVAL;
        return -1;
    }

    if (dirname != NULL) {
        perror("appendToFile con dirname!= NULL non supportata");
        errno = ENOSYS;
    }
...
int closeFile(const char* pathname){
    ...
    if (pathname == NULL) {
        perror("closeFile: argomento illegale");
        errno =EINVAL;
        return -1;
    }
